...
async def test_search_replaces_zone_numbers_in_response_with_string_values(
    mocker, test_async_client, httpx_mock, mock_project, mock_metadata_item, has_permission_true
):
...
async def test_search_overrides_search_service_query_params_admin(
    ...
    mocker.patch(
        ...
        return_value={'role': 'member', 'username': role, 'realm_roles': [f'{project_code}-{role}']},
    )
    ...
    expected_query = f'{expected_query}&container_type=project&container_code={project_code}'
    httpx_mock.add_response(
        method='GET',
...
async def test_search_overrides_search_service_query_params_collab(
    ...
    mocker.patch(
        'app.auth.get_current_identity',
        return_value={'role': role, 'username': role, 'realm_roles': [f'{project_code}-{role}']},
    )
...
async def test_size_endpoint_returns_search_service_response_admin(
    ...
    mocker.patch(
        'app.auth.get_current_identity',
        return_value={'role': 'member', 'username': user, 'realm_roles': [f'{project_code}-{role}']},
    )
    ...
    url = f'{ConfigClass.SEARCH_SERVICE}/v1/project-files/{project_code}/size'
...
async def test_size_endpoint_returns_search_service_response_collab(
    ...
    mocker.patch(
        'app.auth.get_current_identity',
        return_value={'role': role, 'username': user, 'realm_roles': [f'{project_code}-{role}']},
    )
...
async def test_statistics_endpoint_returns_search_service_response_limited_admin(
    ...
    httpx_mock.add_response(
        method='GET',
        url=re.compile(rf'^{ConfigClass.SEARCH_SERVICE}/v1/project-files/{project_code}/statistics\?zone=0'),
        json=search_response[ConfigClass.GREENROOM_ZONE_LABEL],
    )
    httpx_mock.add_response(
        method='GET',
        url=re.compile(rf'^{ConfigClass.SEARCH_SERVICE}/v1/project-files/{project_code}/statistics\?zone=1'),
        json=search_response[ConfigClass.GREENROOM_ZONE_LABEL],
    )
...
async def test_statistics_endpoint_returns_search_service_response_limited_to_role_collab(
    ...
    httpx_mock.add_response(
        method='GET',
        url=re.compile(
            rf'^{ConfigClass.SEARCH_SERVICE}/v1/project-files/{project_code}/statistics\?zone=0&parent_path={role}%'
        ),
        json=search_response[ConfigClass.GREENROOM_ZONE_LABEL],
    )
    httpx_mock.add_response(
...
async def test_activity_endpoint_returns_search_service_response_limited_admin(
    role, mocker, test_async_client, httpx_mock, mock_project, has_admin_file_permission
):
    project_code = 'test_project'
    url = (
        f'{ConfigClass.AUTH_SERVICE}authorize?role=admin&resource=project&zone=%2A&operation=view'
    ...
    mocker.patch(
        'app.auth.get_current_identity',
        return_value={'role': 'member', 'username': 'test', 'realm_roles': [f'{project_code}-{role}']},
    )
    mocker.patch(
...
async def test_activity_endpoint_returns_search_service_response_limited_collab(
    role, mocker, test_async_client, httpx_mock, mock_project, has_collab_file_permission, jwt_token_collab
):
    project_code = 'test_project'
    url = (
        f'{ConfigClass.AUTH_SERVICE}authorize?role=collaborator&resource=project&zone=%2A&operation=view'
    ...
    httpx_mock.add_response(
        method='GET',
        url=re.compile(rf'^{ConfigClass.SEARCH_SERVICE}/v1/project-files/{project_code}/activity\?user=test'),
        json=expected_response,
    )
...
async def test_activity_endpoint_returns_search_service_response_limited_contrib(
    role, mocker, test_async_client, httpx_mock, mock_project, has_contrib_file_permission, jwt_token_contrib
):
    project_code = 'test_project'
    expected_response = {
        'data': {
...
async def test_activity_endpoint_returns_search_service_response_limited_to_role_invalid_project_permissions(
    role, mocker, test_async_client, httpx_mock, mock_project
):
    project_code = os.urandom(6).hex()
    mocker.patch(
        'app.auth.get_current_identity',
